...
\large A molecular fragmentation world reveals the dark metabolome}
\author{}
\date{}
...
\subsection{Closed chemical state}
\methodtodo{Bond--electron state; atom identity; explicit H; formal charge/radicals; exact mass; ionization/adduct state; charged and neutral connected components; acquisition conditions; canonical state identity.}

\subsection{Autoregressive single-electron event language}
\methodtodo{Source selection from atoms/occupied bonds; atom-versus-bond target decision; sequential target atom pointers; 1e transfer; learned STOP/REPEAT/NEW event control; relation between repeated 1e events and conventional 2e pair transfer; accumulation of multi-event transformations before deterministic compilation; absence of reaction-template enumeration.}

\subsection{Chemical compiler and legality}
\methodtodo{Atom/electron/charge/H/mass conservation; bond-electron symmetry; supported valence/materialization checks; treatment of compiler rejection; distinction between executable chemistry and validated mechanism.}

\subsection{Multi-branch reaction DAG and probability mass}
\methodtodo{Sibling reaction branches; branch-local STOP; reconvergent paths; canonical DAG aggregation; frontier scheduling; explicit decomposition of sampled policy mass into valid, invalid, event-budget-censored, unsampled and STOP outcomes; probability-conservation checks; censoring as unresolved rather than chemical failure.}

\subsection{Partial observation semantics}
The experimental tandem mass spectrum is treated as a partial observation of the latent fragmentation World. Legal generated states that are not observed are not automatically labelled false. Likewise, reaction regions that remain unsampled or are censored by computational budget are not interpreted as chemically impossible.
...
\section{Free, Guided and Inverse inference over one World}

\subsection{Shared World representation}
\methodtodo{Shared molecular/fragment graph encoding; task-mode conditioning; spectrum peak-graph encoding when applicable; checkpoint lineage; strict absence of target-spectrum input from Free inference.}

\subsection{Free inference}
\methodtodo{Teacher-free autoregressive electron-event rollout; Subtrajectory Balance/GFlowNet objective; multibranch probability flow; observation-derived positive evidence; ordered-transition endpoints; unresolved/censored probability accounting.}

\subsection{Guided inference}
\methodtodo{Spectrum-conditioned graph policy/value over the same executable World; frontier prioritization under matched compute; auxiliary IQL objective on sampled transitions; exact production checkpoint/inference path. Explicitly state that DAgger and HER are not currently part of production rollout unless implemented before manuscript freeze.}

\subsection{Inverse inference}
\methodtodo{Factorized graph-construction policy; action-kind/element/charge/radical/attachment/pair/bond-order predictions; hard formula/charge/H/valence constraints; canonical partial-state merging; leaf--parent--precursor ordered-fragment shaping; SMC posterior maintenance; completed-candidate forward World replay.}

\subsection{Bidirectional evidence}
\methodtodo{Candidate replay through Free/Guided World; mass/intensity/ordered-transition support; candidate-specific discrimination; unresolved World probability mass; definition of the bidirectional consistency endpoint used in the manuscript.}
