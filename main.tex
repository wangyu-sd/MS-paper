...
\normalfont\small Molecule + ionization/collision condition $\rightarrow$ latent multi-branch fragmentation world $\rightarrow$ observed spectrum. Draw charged and neutral products, low-abundance/unobserved branches and the instrument observation layer. Main equation: $M,c\rightarrow\mathcal{W}(M,c)\rightarrow S_{\rm obs}$.}
\hfill
\paneltodo{0.38\textwidth}{0.16\textheight}{B | One World, three queries\\[3pt]
\normalfont\small Central Fragmentation World with three arrows: Free/prediction $p(\mathcal T,S\mid M,c)$; Guided/explanation $p(\mathcal T\mid M,S_{\rm obs},c)$; Inverse/structure inference $p(M,\mathcal T\mid S_{\rm obs},c)$.}
\\[5pt]
\paneltodo{0.28\textwidth}{0.12\textheight}{C | Autoregressive 1e chemistry\\[3pt]
\normalfont\small Source $\rightarrow$ target $\rightarrow$ 1e; then STOP / REPEAT / NEW. Show two repeated 1e events recovering a 2e transfer and different targets producing radical/rearrangement patterns. No reaction-template catalogue.}
\hfill
\paneltodo{0.30\textwidth}{0.12\textheight}{D | Probability-mass semantics\\[3pt]
\normalfont\small Ledger or flow diagram with $P_{\rm stop}+P_{\rm valid}+P_{\rm invalid}+P_{\rm censored}+P_{\rm unsampled}=1$. Emphasize that invalid, unresolved and true STOP are not redistributed into successful branches.}
\hfill
\paneltodo{0.28\textwidth}{0.12\textheight}{E | Multi-branch reaction DAG\\[3pt]
\normalfont\small One precursor, several sibling reactions, branch-local STOP and reconvergent paths into a common canonical fragment state. Annotate conditional/path probabilities and summed DAG flow.}
\\[5pt]
\paneltodo{0.90\textwidth}{0.065\textheight}{F | Outer-loop scientific evolution\\[3pt]
\normalfont\small Thin strip only: systematic W/G/V failure $\rightarrow$ bounded agent proposal $\rightarrow$ typed chemistry compiler + conservation checks $\rightarrow$ molecule-disjoint held-out gate $\rightarrow$ promoted World. Agent remains outside online inference.}
...
\normalfont\small Reaction-network example plus held-out distributions/curves for observed mass coverage, intensity-weighted coverage, ordered-transition coverage, multistage gain and probability conservation. Show network-level evidence, not only spectral cosine.}
\hfill
\paneltodo{0.43\textwidth}{0.13\textheight}{B | Guided explanation under matched compute\\[3pt]
\normalfont\small Paired Free vs Guided coverage-vs-expansions curves. Same molecule, legal chemistry and budget; only Guided sees the spectrum. Report intensity/path gain and states or wall time needed for matched coverage.}
\\[5pt]
\paneltodo{0.38\textwidth}{0.14\textheight}{C | Inverse posterior structures\\[3pt]
\normalfont\small Spectrum $\rightarrow$ several candidate molecules with posterior weights/evidence. Report Exact/Recall@K, best structural similarity, validity and diversity/ESS. Show multiple modes, not only Top-1.}
\hfill
\paneltodo{0.48\textwidth}{0.14\textheight}{D | Bidirectional consistency\\[3pt]
\normalfont\small Closed loop $S_{\rm obs}\rightarrow M\rightarrow\mathcal W(M)\rightarrow\hat S$. Compare GT and hard same-formula/near-isomer decoys using forward intensity/path support, ordered transitions and unresolved World mass.}
\\[5pt]
\paneltodo{0.54\textwidth}{0.12\textheight}{E | Graded structural evidence\\[3pt]
\normalfont\small Horizontal ladder/funnel: formula compatibility $\rightarrow$ valid graph $\rightarrow$ World reachability $\rightarrow$ executable path support $\rightarrow$ candidate-specific evidence $\rightarrow$ bidirectional consistency $\rightarrow$ calibrated bounded statement $\rightarrow$ standard confirmation. Overlay reliability and coverage at each level.}
\hfill
\paneltodo{0.32\textwidth}{0.12\textheight}{F | Prospective credibility\\[3pt]
\normalfont\small Freeze-date timeline plus calibration plot: dark at freeze $\rightarrow$ independently deposited later. Report containment, calibration error and performance versus structural distance; include fair calibrated comparators.}
...
\normalfont\small Full-width Sankey/funnel from all dark spectra through formula-compatible, World-reachable, executable-path, candidate-specific, bidirectionally consistent and calibrated bounded-structure levels. Keep unresolved and computationally censored spectra visible at every stage; show denominators.}
\\[5pt]
\paneltodo{0.42\textwidth}{0.12\textheight}{B | Spectral observations to structural entities\\[3pt]
\normalfont\small Sankey/flow for adduct, isotope, charge-state, in-source-fragment and redundant-acquisition collapse. Report validation error and entity-count uncertainty.}
\hfill
\paneltodo{0.42\textwidth}{0.12\textheight}{C | Recurrence across independent datasets\\[3pt]
\normalfont\small ECDF/distribution of entity/family recurrence over independent datasets or laboratories. Distinguish technical repetition from independent recurrence.}
\\[5pt]
\paneltodo{0.42\textwidth}{0.12\textheight}{D | Evidence strength versus recurrence\\[3pt]
\normalfont\small Paired distribution/ordinal trend showing strongest evidence level as a function of recurrence. Test whether recurrent chemistry is structurally more resolvable than singletons.}
\hfill
\paneltodo{0.42\textwidth}{0.12\textheight}{E | Near-known versus remote chemistry\\[3pt]
\normalfont\small Distance-to-known distribution and partition of high-evidence recurrent entities into near-known halo, remote recurrent families and insufficient-resolution cases. Frozen reference space and threshold.}
\\[5pt]
\paneltodo{0.90\textwidth}{0.075\textheight}{F | Representative high-evidence dark chemistry\\[3pt]
\normalfont\small Horizontal strip of 3--5 examples: World-resolved candidate; bounded isomer set; remote recurrent family member; standard-confirmed anchor if available. Each carries evidence level, recurrence and remaining uncertainty.}
...
\normalfont\small Large structure-based map/graph containing characterized metabolites and high-evidence recurrent dark entities. Mark known neighbourhood, near-known halo and remote recurrent islands. Avoid a decorative hairball; show density/family boundaries.}
\hfill
\paneltodo{0.34\textwidth}{0.16\textheight}{B | Recurrent family architecture\\[3pt]
\normalfont\small Family-size and recurrence distributions, emphasizing repeated remote families versus isolated singletons. Include family/evidence-level composition rather than bars only.}
\\[5pt]
\paneltodo{0.90\textwidth}{0.07\textheight}{C | Public perturbation evidence\\[3pt]
\normalfont\small Thin horizontal design strip: manually verified germ-free/gnotobiotic, antibiotic and defined-diet cohorts; show study/sample counts and platforms. This panel establishes the attribution evidence base.}
\\[5pt]
\paneltodo{0.53\textwidth}{0.15\textheight}{D | Source dependence over chemical space\\[3pt]
\normalfont\small Reuse the structural map from A and overlay microbiota-dependent, diet-dependent, host-associated, mixed and unresolved families. The visual question is whether source labels partition chemical space.}
\hfill
\paneltodo{0.34\textwidth}{0.15\textheight}{E | Structure--source coupling\\[3pt]
\normalfont\small Quantitative distributions for local label autocorrelation, motif/class enrichment and cross-study effect consistency with permutation/null intervals.}
\\[5pt]
\paneltodo{0.90\textwidth}{0.085\textheight}{F | Worked remote family\\[3pt]
\normalfont\small Family structures + recurrence across datasets + perturbation effects + source label + evidence level. Use dependent/associated, not biosynthesized, unless direct biosynthetic evidence exists.}
...
\normalfont\small Paired analysis of the same samples/covariates as anonymous dark features versus frozen structural families. Show the preregistered primary endpoint, preferably cross-cohort replication or effect-sign concordance.}
\hfill
\paneltodo{0.42\textwidth}{0.135\textheight}{B | Reproducibility gain\\[3pt]
\normalfont\small Paired distributions across all eligible contrasts/families for replication, sign concordance, multiplicity-controlled yield and effect stability. Avoid a single cherry-picked positive case.}
\\[5pt]
\paneltodo{0.39\textwidth}{0.135\textheight}{C | Principal biological programme\\[3pt]
\normalfont\small Sample-level effect plot plus cohort-level estimates for one strongest frozen family after the global analysis is fixed. Include adjusted significance and independent replication.}
\hfill
\paneltodo{0.47\textwidth}{0.135\textheight}{D | Family chemistry\\[3pt]
\normalfont\small Molecular structures/bounded statements of family members with evidence level, confidence, recurrence and interpretable structural differences. Do not imply a biochemical pathway from structural adjacency alone.}
\\[5pt]
\paneltodo{0.42\textwidth}{0.13\textheight}{E | Authentic-standard anchors\\[3pt]
\normalfont\small Sample-versus-standard MS/MS overlay and retention/coelution for preselected anchors. Show acceptance criteria; failed anchors remain in the Extended Data denominator.}
\hfill
\paneltodo{0.42\textwidth}{0.13\textheight}{F | Explicitly unresolved member\\[3pt]
\normalfont\small Surviving isomer set, evidence that eliminated alternatives and the additional measurement predicted to distinguish the remaining candidates. Make uncertainty visible rather than forcing a unique identity.}
